...
PivotViewer.Version="0.9.178-8be0d74";
PivotViewer.Models = {};
...
PivotViewer.Models.Item = Object.subClass({
	init: function (Img, Id, Href, Name) {
		this.Img = isNaN(parseInt(Img)) ? Img : parseInt(Img),
		this.Id = Id,
		this.Href = Href,
		this.Name = Name,
		this.Description,
...
PivotViewer.Models.FacetValue = Object.subClass({
	init: function (Value) {
		this.Value = typeof Value === 'boolean' ? Value.toString() : Value;
		this.Href = "";
	}
});
...
PivotViewer.Views.GraphView = PivotViewer.Views.TileBasedView.subClass({
    ...
    Filter: function (dzTiles, currentFilter, sortFacet, stringFacets, changingView, changeViewSelectedItem) {
        ...
        this.tiles = dzTiles.sort(this.SortBy(this.sortFacet, false, function (a) {
            return a ? ($.isNumeric(a) ? a : a.toUpperCase()) : "";
        }, stringFacets));
                           ...
                           for (l = 0; l < this.tiles[j].facetItem.Facets[k].FacetValues.length; l++) {
                              ...
                              if (this.IsFilterVisible (attribute)) {
                                  ...
                                  if (this.tiles[j].facetItem.Href) { 
                                      ...
                                      // Value is uri
                                      if (this.IsUri(value))
                                          tableRows.push({key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven +"'><td id='pv-key' class='tooltipcustom' title='Select this value'>" + entity + "</td><td id='pv-facet' class='tooltipcustom' title='Show only this relation'> style='color:#009933;cursor:pointer'" + attribute + "</td><td id='pv-value'><a href" + value + ">" + value + "</a></td></tr>"});
                                       else
                                          tableRows.push({key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven +"'><td id='pv-key' class='tooltipcustom' title='Select this value'>" + entity + "</td><td id='pv-facet' class='tooltipcustom' title='Show only this relation'> style='color:#009933;cursor:pointer'" + attribute + "</td><td id='pv-value'>" + value + "</td></tr>"});
                                 }
                             } 
                             if (oddOrEven == 'odd-row')
                                 oddOrEven = 'even-row';
                               ...
                               for (l = 0; l < this.tiles[j].facetItem.Facets[k].FacetValues.length; l++) {
                                  ...
                                  if (this.IsFilterVisible (attribute)) {
                                      ...
                                      if (this.tiles[j].facetItem.Href) { 
                                          ...
                                          // Value is uri
                                          if (this.IsUri(value))
                                              tableRows.push({key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven +"'><td id='pv-key' class='tooltipcustom' title='Toggle item selection'>" + entity + "</td><td id='pv-facet' class-'tooltipcustom' title='Clear relation selection' style='color:#009933;cursor:pointer'>" + attribute + "</td><td id='pv-value'><a href" + value + ">" + value + "</a></td></tr>"});
                                          else
                                              tableRows.push({key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven +"'><td id='pv-key' class='tooltipcustom' title='Toggle item selection'>" + entity + "</td><td id='pv-facet' class-'tooltipcustom' title='Clear relation selection' style='color:#009933;cursor:pointer'>" + attribute + "</td><td id='pv-value'>" + value + "</td></tr>"});
                                   }
                               } 
                               if (oddOrEven == 'odd-row')
                                   oddOrEven = 'even-row';
...
//        }
    ...
    DrawRallyCard: function(loc){
        ...
        for(var i=0; i<this.facetItem.Facets.length; i++)
        {
            var currentFacet = this.facetItem.Facets[i];
    ...
    _drawText: function(text, tileObj, x, y){
        tileObj.context.font = '12px Arial';
        tileObj.context.fillStyle = 'black';
        tileObj.context.fillText(text, tileObj._locations[tileObj.selectedLoc].x+x, tileObj._locations[tileObj.selectedLoc].y+y);
    },
    ...
    AttachEventHandlers = function () {
        ...
        $('.pv-filterpanel-search').on('keyup', function (e) {
            ...
            for (var i = 0, _iLen = _wordWheelItems.length; i < _iLen; i++) {
                var wwi = _wordWheelItems[i].Value ? _wordWheelItems[i].Value.toLowerCase() : "";
